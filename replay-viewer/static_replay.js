// Cogchemists static replay shell: fetches the replay named by ?replay=<url>,
...
(function () {
  ...
  function start(module, bytes) {
    ...
    var ok = module._cc_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._cc_error_ptr(),
        module._cc_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._cc_payload_ptr(),
        module._cc_payload_len())
    );
    var loading = document.getElementById("loading");
    if (loading) loading.style.display = "none";
    document.documentElement.removeAttribute("data-replay-error");
    CogchemistsRenderer.attachReplay({
    ...
    // "ready" must mean A PICTURE, not merely a parsed payload: the
    // renderer sets data-replay-loaded at the end of its FIRST DRAWN frame,
    // so poll for that attribute instead of counting animation frames. The
    // poll is bounded, otherwise a renderer that never draws would leave the
    // host waiting forever with no error (cogame-lantern, 2026-08-23).
    var frames = 0;
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = CogchemistsReplayModule().catch(function (error) {

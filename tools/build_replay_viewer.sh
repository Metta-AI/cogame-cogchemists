...
#
# `coworld build` hard-requires this file to be os.X_OK, so it is committed
# mode 100755 and ci.yml asserts the bit before invoking it BY PATH.
set -euo pipefail
...
# `coworld build` pre-creates the parent; a fresh CI checkout does not, and
# resolving the output path below would then fail (ecos, 2026-08-23).
...
test -s "${dist}/cogchemists_replay.wasm"
test -s "${dist}/cogchemists_replay.js"

rm -rf "${output_dir}"
mkdir -p "${output_dir}/assets"
cp "${dist}/cogchemists_replay.js" "${dist}/cogchemists_replay.wasm" \
  "${output_dir}/"
...
echo "cogchemists replay viewer bundle: ${output_dir}"

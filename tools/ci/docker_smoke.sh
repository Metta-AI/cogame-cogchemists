...
# Substitute: cogchemists, coworld-cogchemists, 4.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-cogchemists:ci)
#   SMOKE_SLUG                 game slug                        (cogchemists)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/cogchemists)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/cogchemists-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-cogchemists:ci}}"
slug="${SMOKE_SLUG:-cogchemists}"
...
# Every PLAYER container must also exit 0. Hosted certification checks this
# and the starter smoke did not: whisky raises on a close frame, so a player
# that does not treat a dead socket as a normal episode end scores the seat
# player_error on one dispatch and passes on the next (raid 0.1.3,
# 2026-08-23). A player still running here is also a failure: the game has
# exited, so it should have seen its final frame.
player_failures=0
for ((slot = 0; slot < seats; slot++)); do
  waited=0
  while docker ps -q --filter "name=${prefix}-p${slot}" | grep -q .; do
    if (( waited > 30 )); then break; fi
    sleep 2
    waited=$((waited + 2))
  done
  if docker ps -q --filter "name=${prefix}-p${slot}" | grep -q .; then
    echo "FAIL: player ${slot} container is still running 30s after the game exited" >&2
    player_failures=1
    continue
  fi
  pcode="$(docker inspect -f '{{.State.ExitCode}}' "${prefix}-p${slot}")"
  if [ "${pcode}" != "0" ]; then
    echo "FAIL: player ${slot} container exited ${pcode}" >&2
    player_failures=1
  fi
done
if [ "${player_failures}" != "0" ]; then
  dump_logs
  exit 1
fi
